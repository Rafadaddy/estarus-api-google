...
import { Bus, Download, Trash2, Plus, Search, Hash, RefreshCw, Cloud, HardDrive } from "lucide-react";
...
export default function HybridHome() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<"testing" | "connected" | "local">("testing");
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const localData = await localStorageAPI.getAllBusUnits();
      setUnits(localData);
      ...
      if (localData.length === 0) {
        toast({
          ...
          description: "Aplicación lista. Agrega tu primera unidad.",
        });
      } else {
        toast({
          title: "Datos Cargados",
          description: `${localData.length} unidades encontradas`,
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Error al cargar los datos locales",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleAddUnit = async () => {
    ...
    try {
      ...
      // Always use local storage for now since Google Sheets API keys don't support writing
      const newUnit = await localStorageAPI.createBusUnit(unitData);
      ...
      toast({
        title: "Éxito",
        description: `Unidad ${number} agregada exitosamente.`,
      });
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleToggleStatus = async (unit: BusUnit, component: string) => {
    ...
    setIsUpdating(true);
    try {
      const updatedUnit = await localStorageAPI.updateBusUnit(unit.id, {
        [component]: newStatus
      });
  ...
  const handleDeleteUnit = async (unitId: number) => {
    setIsUpdating(true);
    try {
      const success = await localStorageAPI.deleteBusUnit(unitId);
  ...
  const handleDeleteAll = async () => {
    setIsUpdating(true);
    try {
      await localStorageAPI.deleteAllBusUnits();
      setUnits([]);
      toast({
        title: "Éxito",
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Actualizando",
      description: "Verificando conexión y cargando datos...",
    });
  };
  ...
  const getStorageIcon = () => {
    if (connectionStatus === "testing") return <RefreshCw className="h-4 w-4 animate-spin" />;
    if (connectionStatus === "connected") return <Cloud className="h-4 w-4 text-green-400" />;
    return <HardDrive className="h-4 w-4 text-orange-400" />;
  };

  const getStorageText = () => {
    if (connectionStatus === "testing") return "Conectando...";
    if (connectionStatus === "connected") return "Google Sheets";
    return "Local";
  };
          ...
          <div className="flex items-center justify-between h-12 sm:h-16">
            <div className="flex items-center">
              ...
              <div className="flex items-center text-white/70 text-xs ml-2">
                {getStorageIcon()}
                <span className="ml-1 hidden sm:inline">{getStorageText()}</span>
              </div>
            </div>
            ...
            {isLoading ? (
              <div className="flex items-center justify-center h-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-sm">Verificando conexión...</p>
                </div>
              </div>
