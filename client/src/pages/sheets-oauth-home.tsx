...
import { googleSheetsOAuthAPI } from "@/lib/googleSheetsOAuth";
...
import { Bus, Download, Trash2, Plus, Search, Hash, RefreshCw, Cloud, Shield, UserCheck } from "lucide-react";
...
export default function SheetsOAuthHome() {
  ...
  // Initialize Google API and check authentication
  const initializeAPI = async () => {
    setIsInitializing(true);
    try {
      await googleSheetsOAuthAPI.initialize();
      setIsAuthenticated(googleSheetsOAuthAPI.isAuthenticated());
      
      if (googleSheetsOAuthAPI.isAuthenticated()) {
        await loadData();
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "No se pudo inicializar la API de Google",
        variant: "destructive",
      });
    } finally {
  ...
  const handleAuthenticate = async () => {
    setIsLoading(true);
    try {
      const success = await googleSheetsOAuthAPI.authenticate();
      if (success) {
        setIsAuthenticated(true);
        await loadData();
        toast({
          title: "Autenticación Exitosa",
          description: "Conectado a Google Sheets. Los datos se compartirán entre usuarios.",
        });
      } else {
        toast({
  ...
  const loadData = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await googleSheetsOAuthAPI.getAllBusUnits();
      setUnits(data);
      
      if (data.length === 0) {
        toast({
          title: "Google Sheets Conectado",
          description: "Conexión exitosa. Agrega tu primera unidad.",
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "Error al cargar datos de Google Sheets",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleAddUnit = async () => {
    if (!isAuthenticated) {
      toast({
        ...
        description: "Debes autenticarte con Google primero",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const newUnit = await googleSheetsOAuthAPI.createBusUnit(unitData);
      ...
      toast({
        title: "Error",
        description: "Error al agregar la unidad",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  const handleToggleStatus = async (unit: BusUnit, component: string) => {
    ...
    setIsUpdating(true);
    try {
      const updatedUnit = await googleSheetsOAuthAPI.updateBusUnit(unit.id, {
        [component]: newStatus
      });
  ...
  const handleDeleteUnit = async (unitId: number) => {
    ...
    setIsUpdating(true);
    try {
      const success = await googleSheetsOAuthAPI.deleteBusUnit(unitId);
  ...
  const handleDeleteAll = async () => {
    ...
    setIsUpdating(true);
    try {
      await googleSheetsOAuthAPI.deleteAllBusUnits();
      setUnits([]);
      toast({
        title: "Éxito",
  ...
  const handleSignOut = async () => {
    await googleSheetsOAuthAPI.signOut();
    setIsAuthenticated(false);
    setUnits([]);
    toast({
      title: "Sesión Cerrada",
      description: "Has cerrado sesión de Google Sheets",
    });
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Inicializando Google API</h2>
          <p className="text-gray-500">Configurando conexión con Google Sheets...</p>
        </div>
      </div>
    );
  }
        ...
        <nav className="bg-blue-600 shadow-lg">
          <div className="max-w-7xl mx-auto px-4 py-4">
            <div className="flex items-center">
              ...
              <span className="text-white font-semibold text-lg">Gestión de Unidades</span>
              <span className="text-white/70 text-sm ml-2">Google Sheets</span>
            </div>
          </div>
        </nav>
          ...
          <Card className="border-2 border-blue-200">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Para usar la aplicación con datos compartidos, necesitas autenticarte con Google Sheets.
              </p>
              <Button 
              ...
              <p className="text-xs text-gray-500 mt-4">
                Se abrirá una ventana para autorizar el acceso a Google Sheets
              </p>
            </CardContent>
          </Card>
        </div>
