...
import { Bus, Download, Trash2, Plus, Search, Hash, RefreshCw } from "lucide-react";
...
export default function SheetsHome() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await googleSheetsAPI.getAllBusUnits();
      console.log("Data loaded successfully:", data);
      setUnits(data);
      if (data.length === 0) {
        console.log("No data found, but connection successful");
      }
    } catch (error) {
      console.error("Failed to load data:", error);
      toast({
        title: "Error de Conexión",
        description: "No se puede conectar con Google Sheets. Verificando permisos...",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleAddUnit = async () => {
    ...
    setIsUpdating(true);
    try {
      const newUnit = await googleSheetsAPI.createBusUnit({
        unitNumber: number,
        MOT: "listo",
        TRAN: "listo",
        ELE: "listo",
  ...
  const handleToggleStatus = async (unit: BusUnit, component: string) => {
    ...
    setIsUpdating(true);
    try {
      const updatedUnit = await googleSheetsAPI.updateBusUnit(unit.id, {
        [component]: newStatus
      });
  ...
  const handleDeleteUnit = async (unitId: number) => {
    setIsUpdating(true);
    try {
      const success = await googleSheetsAPI.deleteBusUnit(unitId);
      if (success) {
  ...
  const handleDeleteAll = async () => {
    setIsUpdating(true);
    try {
      await googleSheetsAPI.deleteAllBusUnits();
      setUnits([]);
      toast({
        title: "Éxito",
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Actualizando",
      description: "Cargando datos más recientes...",
    });
  };
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4">
          <div className="flex items-center justify-between h-12 sm:h-16">
            <div className="flex items-center">
              ...
              <span className="text-white/70 text-xs ml-2 hidden sm:inline">(Datos Compartidos)</span>
            </div>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ¿Está seguro de que desea eliminar todas las unidades? Esta acción afectará a todos los usuarios y no se puede deshacer.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
            ...
            {isLoading ? (
              <div className="flex items-center justify-center h-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-sm">Cargando datos compartidos...</p>
                </div>
              </div>
